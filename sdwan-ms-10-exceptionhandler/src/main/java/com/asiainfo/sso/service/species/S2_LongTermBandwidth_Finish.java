 ...
 */
@Service(value = "S2-LongTermBandwidth_Finish")
public class S2_LongTermBandwidth_Finish extends Data_BaseClass {
    ...
    //盒子的长期带宽属性表current_lt_box修改
    public void current_lt_box() throws Exception {
        for (So_ord_site_box obj : so_ord_site_boxLs) {
            Current_lt_box inst = new Current_lt_box();
            inst.setProd_inst_id(so_prod_inst.getProd_inst_id());
            inst.setSite_name(obj.getSite_name());
            inst.setYlw_site_id(obj.getYlw_site_id());
            inst.setBox_bandwith(obj.getBox_bandwith());
            inst.setStart_time(obj.getStart_time());
            prodClientService.updateCurrent_lt_box(inst);
            Logs.createLogs("Current_lt_box updateDB:" + inst.toString());
